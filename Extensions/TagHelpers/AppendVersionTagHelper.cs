...
namespace Itsomax.Module.Core.Extensions.TagHelpers
{
    [HtmlTargetElement(Attributes = AppendVersionAttributeName)]
    public class AppendVersionTagHelper : TagHelper
    {
        private const string AppendVersionAttributeName = "itsomax-append-version";
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (output.Attributes.ContainsName("ItsomaxCms"))
            {
                var src = output.Attributes["ItsomaxCms"].Value.ToString();
                output.Attributes.SetAttribute("ItsomaxCms", AppendVersionToUrl(src));
            }
        }
